...
<div class="animate-slide-up {status === 'sending' ? 'opacity-65' : ''}">
  {#if sender === "ai"}
    <div class="flex gap-3 items-end max-w-3xl">
      <div
        class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 self-start shadow-lg"
      >
        <img src="/robot.png" alt="AI" class="w-8 h-8 object-contain" />
      </div>
      <div
        class="glass flex-1 px-5 py-4 rounded-3xl rounded-bl-md shadow-glass"
      >
        <div
          class="text-[0.9375rem] text-slate-700 markdown-content leading-relaxed"
        >
  ...
  {:else}
    <div class="flex justify-end">
      <div class="relative flex flex-col items-end max-w-2xl">
        <div
